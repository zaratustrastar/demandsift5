  ...
  async analyzeBusiness(request: AnalyzeBusinessRequest): Promise<AiProviderResult<BusinessUnderstanding>> {
    ...
    return this.structured({
      ...
      system:
        ...
        "customerProblemLanguage should contain exactly 5 natural pain phrases (no more), each a short self-contained clause (roughly 5-8 words) phrased the way a real person would actually say or type it -- the kind of wording that could plausibly appear verbatim in a Reddit post or comment, not a compressed keyword label. Downstream retrieval already turns these into Reddit search queries, so do not pre-shorten or pre-optimize the wording for search here; just describe the lived problem naturally, e.g. \"kids watching TV way past bedtime\" or \"can't lock the TV remotely\", not \"TV volume cap\" or \"different weekend screen limits\". Prefer concrete manifestations such as files buried in email, missed client deadlines, scattered client work, unclear task ownership or too many tools when those meanings are supported. Do not present these phrases as observed customer quotations. Avoid formal marketing prose. " +
