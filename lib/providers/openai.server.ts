  ...
  async analyzeBusiness(request: AnalyzeBusinessRequest): Promise<AiProviderResult<BusinessUnderstanding>> {
    ...
    return this.structured({
      ...
      system:
        ...
        "customerProblemLanguage should contain exactly 5 search-ready problem concepts (no more) -- not raw customer complaint sentences, and not something you expect later code to shorten or rewrite. Downstream code only lowercases, strips punctuation and URL-encodes these verbatim; it does no stopword removal, truncation or semantic rewriting, so each entry must already be the exact natural-language phrase DemandSift should search on Reddit. " +
        "For each problem, decide first whether the problem wording alone already identifies the market it belongs to. If it does (e.g. \"customers abandoning checkout\" already says ecommerce, \"employees keep forgetting timesheets\" already says timesheet tracking), the concept can be close to that natural phrasing on its own. If the problem is ambiguous outside this specific business (e.g. \"can't see project status\", \"can't lock it remotely\") attach the smallest useful market/category anchor -- normally productCategory's own wording -- so the concept reads unambiguously without needing outside context. Prefer 3-6 words. " +
        "Examples of the transformation this field must already reflect, not defer to later code: \"projects scattered across tools\" -> \"projects scattered tools\" (self-identifying, just tightened); \"can't see project status\" -> \"project management project status\" (ambiguous alone, anchored to the category); \"customers abandoning checkout\" -> \"ecommerce checkout abandonment\"; \"employees keep forgetting timesheets\" -> \"employee timesheet tracking\"; a TV parental-control business's \"can't lock the TV remotely\" -> \"tv parental control remote lock\" (market anchor plus problem core, not the raw complaint sentence and not a mechanically word-deleted fragment of it). " +
